# SIGA — Atividade de Padrões Criacionais (código inicial)

**Técnicas de Programação II (TP2) · Aula 6** — CST em Desenvolvimento de Software Multiplataforma · Fatec de Porto Ferreira

Este é o **código inicial** da atividade prática da Aula 6. Ele contém, de forma **proposital**, três problemas de design que você deverá resolver aplicando, em conjunto, os padrões **Abstract Factory**, **Builder** e **Singleton**. O programa compila e executa — o problema não é o funcionamento, e sim a coerência, a legibilidade e o controle da criação de objetos.

## Estrutura do projeto

```
siga-criacionais/
└── src/
    └── siga/
        ├── Conexao.java              (interface — produto; pronta)
        ├── Comando.java              (interface — produto; pronta)
        ├── ObjetosAcessoDados.java   (implementações MySQL e PostgreSQL; prontas)
        ├── AcessoDados.java          (contém os três problemas a refatorar)
        └── Main.java                 (demonstra os problemas em execução)
```

## Como compilar e executar

Pré-requisito: JDK 17 ou superior (`java -version` para verificar).

```bash
# 1. Compilar (a saída vai para a pasta "bin")
javac -d bin src/siga/*.java

# 2. Executar
java -cp bin siga.Main
```

## Os problemas propositais

| Local | Problema | Padrão que resolve |
|---|---|---|
| `AcessoDados.conectar` | Cria conexão e comando por `if` e `new` separados, sem garantir que sejam do mesmo fornecedor (dá para misturar MySQL e PostgreSQL). | **Abstract Factory** |
| `AcessoDados.montarConsulta` | Método com muitos parâmetros opcionais (construtor telescópico), ilegível e propenso a erro de ordem. | **Builder** |
| `AcessoDados` | Nada garante um único ponto de acesso ao banco no sistema. | **Singleton** |

## Sua tarefa

Siga as etapas da ficha de atividade prática:

1. **Analisar** o código inicial e identificar a possibilidade de misturar fornecedores e o método de consulta telescópico.
R: Bom, temos a mistura de conexões e comandos entre MySQL e PostgreSQL em "AcessoDados" devido a suas criações independentes dentro do mesmo método, não regulados por nenhuma regra que defina a necessidade de caso uma conexão for PostgreSQL então o comando também deve ser PostgreSQL. Juntamente com ambos fornecedores tendo suas conexões e comandos sob os mesmo tipos ("Conexao" e "Comando"), podem ser atribuídos isoladamente sem erro pois não há nenhuma expressão de relação indicando o contrário em qualquer lugar. Por fim, este problema somente poderia ser constatado na execução, após a criação do objeto incoerente e quando dependeríamos da combinação correta entre a conexão e comando, muito tarde e distante da causa.
Já no problema do "telescópio" temos os 7 parâmetros de "montarConsulta", dos quais somente 1 é obrigatório, sendo todos os demais opcionais. Além de que na chamada "acesso.montarConsulta(...)" no "Main", temos uma sequência numérica ("50","0" e "30") sem significado aparente que nos obriga a consultar no código "montarConsulta" novamente para ter noção de onde irão estes valores. E também temos um erro nestes mesmos números: podemos alterar a posição deles por todos serem "int" que o código aceita e muda completamente a consulta feita, somente sendo percebido ao checar o real resultado com aquele montado pela chamada. Por fim, "timeoutSegundos" é recebido na chamada mas não usado pois não consta um uso para si no corpo do método "montarConsulta". Isto consta no Item 2 do "Effective Java" de Bloch: quando há muitos parâmetros para construção, é recomendado "Builder".

2. **Abstract Factory:** criar uma fábrica abstrata (por exemplo, `FabricaBanco`) com `FabricaMySQL` e `FabricaPostgreSQL`, cada uma produzindo uma `Conexao` e um `Comando` **do mesmo fornecedor**. O `AcessoDados` passa a receber uma fábrica e criar a família coerente a partir dela.
R: Antes tínhamos os problemas de conexões e comandos criados separadamente dentro de um mesmo método e o compilador aceitava por compatibilidade com os tipos "Conexao" e "Comando". Agora, criamos uma interface "FabricaBanco" para regularizar a criação conjunta de "Conexao" e "Comando", impedimos a junção de uma conexão MySQL com um comando PostgreSQL pelas fábricas concretas "FabricaMySQL" e "FabricaPostgreSQL" para produzirem o par correto via uma única decisão: qual fábrica utilizar dentro de nosso "Main". Também alteramos "AcessoDados" de uma condicional com o parâmetro "String fornecedor" a qual continha 4 instanciações diretas, duas para cada ramo do "if", para chamar nosso par de objetos desejado juntos. Tal se evidencia pela saída com dois blocos coerentes para cada BD e pela troca de fornecedor ser a troca da fábrica no construtor. Tudo previsto pela "Abstract Factory" catalogado como um padrão pelo Gang of Four (1994), porém diferencia do "Factory Method" da aula passada pois este cria somente um produto enquanto o presente em nosso código gera uma família de produtos que necessita combinar entre si.

3. **Builder:** criar um `ConsultaBuilder` com métodos nomeados e encadeáveis para os parâmetros opcionais (`comFiltro`, `comOrdenacao`, `comLimite`, etc.) e um `construir()` que devolve a consulta. Substitui o método telescópico.
4. **Singleton:** transformar o `AcessoDados` em um Singleton, com construtor privado, instância estática e método de acesso.
5. **Desenhar** o diagrama de classes da solução (fábrica de banco, produtos, builder e acesso), evidenciando os três padrões.

## Critério de sucesso

Ao final: (a) deve ser **impossível** combinar uma conexão de um fornecedor com um comando de outro; (b) a montagem da consulta deve ser **legível**, com passos nomeados; e (c) deve existir **um único** ponto de acesso ao banco, obtido de forma controlada.

## Padrão de entrega

Conforme a ficha de atividade prática: identificadores em português, um arquivo `.java` por classe pública, código formatado, entrega no repositório Git com README e commits descritivos. O uso de IA para gerar o código é proibido nesta atividade (ver seção 5.3 da ficha).
